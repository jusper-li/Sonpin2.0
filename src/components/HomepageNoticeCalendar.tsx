    ...
    <div className="sonpin-container sonpin-notice-calendar-grid">
      <div className="sonpin-announcement-panel">
        <div className="sonpin-section-heading">
          <div><p className="sonpin-eyebrow"><Bell size={14} /> 最新公告</p><h2>公告消息</h2></div>
          <Link to="/blog/categories/bulletin" className="sonpin-text-link">查看全部 <ExternalLink size={14} /></Link>
        </div>
        <div className="sonpin-announcement-list">
          {announcements.length > 0 ? announcements.map((article) => <Link to={`/blog/posts/${article.slug}`} className="sonpin-announcement-item" key={article.slug}><span>{article.published_at}</span><strong>{article.title}</strong><ExternalLink size={14} /></Link>) : <p className="sonpin-empty-notice">目前沒有最新公告。</p>}
        </div>
      </div>
      <div className="sonpin-calendar-panel">
        <div className="sonpin-section-heading"><div><p className="sonpin-eyebrow"><CalendarDays size={14} /> 活動行事曆</p><h2>Google 行事曆</h2></div></div>
        {calendarUrl ? <iframe className="sonpin-calendar-frame" src={calendarUrl} title="Google 行事曆" loading="lazy" /> : <div className="sonpin-calendar-empty"><CalendarDays size={32} /><p>尚未設定 Google 行事曆嵌入網址。</p><span>請於 site_settings 建立 google_calendar_embed_url。</span></div>}
      </div>
    </div>
  </section>;
}
