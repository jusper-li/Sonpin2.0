...
export default function HomepageVisual({ sections }: HomepageVisualProps) {
  ...
  return <div className="sonpin-site">
    <SiteHeader />
    <main>
      <section className="sonpin-hero"><div className="sonpin-container sonpin-hero-grid"><div className="sonpin-hero-copy"><p className="sonpin-eyebrow">{hero.label || hero.subtitle}</p><h1 className="whitespace-pre-line">{hero.title}</h1><p className="sonpin-lead">{hero.description || hero.content?.description}</p><div className="sonpin-actions"><Link to={hero.content?.href || '/products'} className="sonpin-button sonpin-button-primary">{t('homepage.cta.products', '商品介紹')} <ArrowRight size={16} /></Link><Link to="/about" className="sonpin-button sonpin-button-ghost">{t('homepage.cta.about', '關於淞品')} <ArrowRight size={16} /></Link></div>{heroBanners.length > 1 && <div className="sonpin-hero-dots" aria-label="Banner 選擇">{heroBanners.map((banner, index) => <button type="button" key={banner.id} aria-label={`切換至 Banner ${index + 1}`} aria-current={index === activeHeroIndex} onClick={() => setActiveHeroIndex(index)} />)}</div>}</div><Link to={hero.content?.href || '/products'} className="sonpin-hero-image"><img src={media(hero)} alt={hero.title} loading="eager" /></Link></div></section>
